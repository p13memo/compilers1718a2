...
class MyParser:
	...
	def expr(self):
		...
		if self.la=='BOOLEAN_VAL_TOKEN':
			if (self.val).lower()=="true" or (self.val).lower()=="t" or self.val=="1":
				self.commandToCalculate = self.commandToCalculate + " 1"
			elif (self.val).lower()=="false" or (self.val).lower()=="f" or self.val=="0":
				self.commandToCalculate = self.commandToCalculate + " 0"
			self.booleanVal()
		elif self.la=='LOG_OP_TOKEN':
			self.commandToCalculate = self.commandToCalculate + " " + self.val
			self.logOp()
	...
	def stringCheck(self):
		...
		#print("val in printCheck: " + self.val)
		if self.la=='string':
			print((self.commandToCalculate).lower())
			varToPrint = eval((self.commandToCalculate).lower())
			print(varToPrint)
			self.match('string')
			self.stmt() 
		else:
